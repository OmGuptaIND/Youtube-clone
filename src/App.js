  ...
  useEffect(() => {
    async function fetchData() {
     await Axios.get('https://www.googleapis.com/youtube/v3/playlistItems' , {
      params:{
        part:'snippet',
        playlistId:"PLINj2JJM1jxNNBnGb3WvHmcyCyfl0VlJX",
        key: KEY, 
        maxResults:50
      }
    }).then(data => {
      setActive(data.data?.items[0]?.snippet?.title)
      setData(data)
      ...
      <div className="yt__head">
        <div className="yt__image">
          <img 
            ...
            className="yt__logo"
          />
          <h3>Created By <b>Om Gupta</b></h3>
        </div>
        <hr />
        <div className="yt__bottom">

          {data.data?.items?.map(res => {
            if (res?.snippet?.title===active){
              return (
                <>
                <Active 
                  key={res?.id}
                  id={res?.id}
                  vedio={res?.snippet?.resourceId?.videoId}
                  alt="err"
          ...
          <div className="yt__margin"></div>
          <div className="yt__suggestions">
          {data.data?.items?.map(res => {
            if (res?.snippet?.title !== active) {
              return (
                <>
                  <Suggestion 
                    img={res?.snippet?.thumbnails?.maxres?.url}
                    alt="Img err"
                    text={res?.snippet?.title}
                    onClick={handleClick}
                  />
